...
#Como se puede observar en la grafica, se realiza un mayor numero de compras en las horas de la tarde. A las 20h se vende un total de 734 productos, siendo el numero mas alto. Y a las 19h un total de 704.
```
...
ggplot(head(top_productos_peso, 10), aes(x = factor(producto, levels = producto), y = num_kg_vendidas)) +
...
df_peso$producto <- gsub("1 ", "", df_peso$producto, fixed = T) #Arreglo un error por el que los productos aparecían de dos formas, por ejemplo, aparecía: "PLATANO" Y "1 PLATANO" como si fueran productos diferentes

head(unique(df_peso$producto), 12) #Vemos los diferentes productos por peso

#Vector de frutas y verduras al peso
Frutas_y_verduras <- c("COL LOMBARDA", "PLATANO", "REPOLLO LISO GRANEL", "CALABACIN VERDE", "PATATA", "TOMATE NEGRO", "PIMIENTO VERDE", "CHIRIMOYA", "TOMATE ENSALADA", "SANDIA BAJA SEMILLAS", "BATATA GRANEL", "BROCOLI", "PIÑA", "PEPINO", "TOMATE CANARIO", "PIMIENTO ROJO", "PARAGUAYO", "MELON PIEL SAPO", "NARANJA", "GRANADA", "TOAMTE RAMA", "BANANA", "MANZANA GOLDEN", "MANDARINA", "PIMIENTO FREIR", "KIWI VERDE", "AGUACATE", "MELOCOTON AMARILLO", "TOMATE ROSA", "BERENJENA RAYADA GR", "MANZANA GRANNY", "MANGO", "LIMON", "TOMATE PERA", "HABAS", "TOMATE VALENCIANO", "PERA CONFERENCIA", "MANZ. ROJA DULCE", "MANZ. ROJA ACIDULCE", "COLIFLOR", "BERENJENA", "NECTARINA", "PERA ROCHA", "POLVORON GRANEL")

frutas_Y_verduras_mas_vendidas <- df_peso %>% mutate(cantidad = gsub(",", ".", cantidad)) %>%  #Cambio la coma por el punto en cantidad para poder pasarlo luego a numeric
...
pescado_mas_vendido <- df_peso %>% mutate(cantidad = gsub(",", ".", cantidad)) %>%
  filter(!producto %in% Frutas_y_verduras) %>% #Filtro por los elementos que NO están en el vector de frutas y verduras del ejercicio anterior
  group_by(producto) %>%
  summarise(Ventas = n(), Kilos = sum(as.numeric(cantidad))) %>%
  arrange(desc(Ventas))
...
ggplot(precio_kilo, aes(x = fecha, y = precio_medio, color=tipo_producto)) + geom_line(size = 1) + geom_point(size = 2) + labs(title = "Variación del precio por kilo en bananas y plátanos", x="Fecha", y="Precio Medio") + theme_minimal() +  scale_color_manual(values = c("Banana" = "salmon", "Plátano" = "green"))
...
#Gracias al gráfico se puede observar que el plátano es mucho más caro en verano y a medida que se acerca el invierno baja de precio.
#Mientras que la banana tiene un precio muy parecido durante todo el año.
```
...
df_tickets %>%
  mutate(dia_semana = wday(as.Date(fecha, "%d/%m/%Y"), label = TRUE, abbr = FALSE, week_start = 1)) %>%
  count(dia_semana) %>%
...
operarios_varias_tiendas <- df_tickets %>%
  distinct(operario, num_tienda, ciudad) %>%  
  group_by(operario) %>%
  summarise(num_tiendas = n(), tiendas = unique(num_tienda), ciudad = ciudad) %>%
