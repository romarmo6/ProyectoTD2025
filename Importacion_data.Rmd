...
top_productos_peso %>%
  slice_min(num_kg_vendidas, n = 10) %>%
  ggplot(aes(x = reorder(producto, num_kg_vendidas), y = num_kg_vendidas)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  coord_flip() +
  labs(
    title = "10 productos vendidos al peso con menor volumen (kg)",
    x = "Producto",
    y = "Kg vendidos"
  ) +
  theme_minimal()
...
operarios_varias_tiendas <- df_tickets %>%
  distinct(operario, num_tienda) %>%  # eliminamos duplicados operario-tienda
  group_by(operario) %>%
