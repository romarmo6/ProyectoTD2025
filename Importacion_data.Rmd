...
leer_ticket <- function(x) {
  ...
  print(lineas_productos)
    ...
    operario = operario,
    factura_comleta = factura_completa,
   ...
   num_ticket = num_ticket,
    ...
    # Creamos un df, la primera columna siendo los nombres de los pdfs, la segunda siendo la fecha de compra, la tercera siendo la hora en cuando se realiza la compra, la cuarta el numero de linea (seq_along),despues le sigue el numero de operario y la factura simplificada, y por último el contenido de cada una de las lineas leidas.
...
# Vector con nombres unicos de ficheros
nombres_unicos <- unique(df_tickets$fichero)

# Direcci
direccion <- df_tickets %>%
  filter(df_tickets$linea==2) %>%
  select(contenido) %>%
  pull() # hacemos pull porque queremos vectores, teniamos antes de esto un df

# CP + Ciudad
linea3 <- df_tickets %>%
  filter(df_tickets$linea==3) %>%
  select(contenido) %>%
  pull()

cp <- substr(trimws(linea3), 1, 5)
ciudad <- substr(trimws(linea3), 7, nchar(linea3))
...
telefono <- substr(trimws(linea4), 11, nchar(linea4))
