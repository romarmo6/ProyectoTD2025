...
>>>>>>> c0a8b66c083be4bed7c00f8a69e8b6e9f0c372db
...
```
Si tuviesemos que cerrar alguna dia de la semana, dando por hecho que los domingos
todos los Mercadonas cierran, cerraria los jueves.
...
operarios_varias_tiendas <- df_tickets %>%
  distinct(operario, num_tienda) %>%  
  group_by(operario) %>%
  summarise(num_tiendas = n(), tiendas = paste(unique(num_tienda), collapse = " - ")) %>%
  filter(num_tiendas > 1)

View(operarios_varias_tiendas)
```
